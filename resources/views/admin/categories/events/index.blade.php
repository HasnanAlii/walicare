<x-admin-layout>
    <div class="py-6 max-w-4xl mx-auto">
        ...
        <div class="mb-4 border-b border-gray-200">
            <nav class="flex -mb-px space-x-6" aria-label="Tabs">
                ...
                <a href="{{ route('admin.categoriesevents.index') }}"
                   class="{{ request()->routeIs('admin.categoriesevents.*') 
                        ? 'border-green-600 text-green-700' 
                        : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}
                          whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors">
                    Kategori Kegiatan
        ...
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-green-900">Daftar Kategori Kegiatan</h2>
            <a href="{{ route('admin.categoriesevents.create') }}" 
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($categoriesevent as $category)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                    <div class="flex justify-center items-center gap-2">
                                        {{-- Tombol Edit --}}
                                        <a href="{{ route('admin.categoriesevents.edit', $category) }}" 
                                        ...
                                        <form id="delete-form-{{ $category->id }}" 
                                              action="{{ route('admin.categoriesevents.destroy', $category) }}" 
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" 
                            ...
                            <tr>
                                <td colspan="3" class="text-center py-12">
                                    <div class="flex flex-col items-center">
                                        <i data-feather="folder" class="w-16 h-16 text-gray-400 mb-4"></i>
                                        <h3 class="text-xl font-semibold text-gray-700">Belum Ada Kategori Kegiatan</h3>
                                        <p class="text-gray-500 mt-2">Buat kategori kegiatan baru dengan menekan tombol "Tambah Kategori".</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            {{-- Pagination --}}
            @if($categoriesevent->hasPages())
                <div class="p-6 bg-gray-50 border-t border-gray-200">
                    {{ $categoriesevent->links() }}
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        function confirmDelete(categoryId) {
            Swal.fire({
                ...
                text: "Kategori kegiatan ini akan dihapus secara permanen.",
                icon: 'warning',
