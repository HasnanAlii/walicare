<x-admin-layout>
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- PERUBAHAN: Desain blok error yang konsisten --}}
            @if ($errors->any())
            ...
            {{-- PERUBAHAN: Membungkus form dalam card --}}
            <div class="bg-white shadow-lg rounded-lg p-6 md:p-8">
                <form action="{{ route('admin.programs.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    {{-- PERUBAHAN: Menambahkan class form yang konsisten --}}
                    @php
                    ...
                    <div class="grid md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            {{-- PERUBAHAN: Input Angka dengan Format --}}
                            <input type="text" name="target_amount" value="{{ old('target_amount') ? number_format(old('target_amount'), 0, ',', '.') : '' }}" class="{{ $formInputClass }}" required oninput="formatInputAsNumber(this)" inputmode="numeric">
                        </div>
                        ...
                        <div>
                            ...
                            {{-- PERUBAHAN: Input Angka dengan Format --}}
                            <input type="text" name="collected_amount" value="{{ old('collected_amount') ? number_format(old('collected_amount'), 0, ',', '.') : '0' }}" class="{{ $formInputClass }}" oninput="formatInputAsNumber(this)" inputmode="numeric">
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        {{-- PERUBAHAN: Input file yang lebih cantik --}}
                    ...
                    <div class="mb-4">
                        ...
                        {{-- PERUBAHAN: Logika 'old' untuk repeater --}}
                        @php
                        ...
                        @if(!empty($old_names))
                            @foreach($old_names as $index => $name)
                                <div class="flex gap-2 items-center">
                                    ...
                                    {{-- Format angka 'old' dan tambahkan oninput --}}
                    ...
                    <div class="mb-6">
                        <label class="flex items-center gap-3">
                            {{-- PERUBAHAN: Checkbox yang konsisten --}}
    ...
    @push('scripts')
    {{-- Script Breakdown Interaktif & Format Angka --}}
    <script>
        // --- FUNGSI FORMAT ANGKA ---
        function formatInputAsNumber(input) {
            // 1. Ambil nilai, hapus semua selain digit
            let value = input.value.replace(/\D/g, '');
            
            // 2. Ubah jadi angka, lalu format ke 'id-ID' (pakai titik)
            let formattedValue = new Intl.NumberFormat('id-ID').format(Number(value));
            
            // 3. Set nilai input ke nilai yang sudah diformat
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Class input yang konsisten
            const inputClass = "border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50";

            function addRow(name = '', amount = '') {
                const div = document.createElement('div');
                div.classList.add('flex', 'gap-2', 'items-center');

                // PERUBAHAN: Menggunakan class, oninput, dan tombol ikon
                div.innerHTML = `
            ...
            // Tambahkan event listener ke tombol 'Hapus' yang sudah ada (dari 'old')
            ...
            form.addEventListener('submit', function(e) {
                ...
                targetAmountInput.value = unformatNumber(targetAmountInput.value);
                collectedAmountInput.value = unformatNumber(collectedAmountInput.value);
                ...
                for(let i=0; i<names.length; i++) {
                    if(names[i].value.trim() !== '') {
                        data.push({
                            ...
                            // Un-format angka breakdown saat mengambil nilainya
